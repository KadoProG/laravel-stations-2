...
<body>
  ...
  <form action="/admin/movies/store" method="POST">
    ...
    <p>画像URL</p>
    <input type="url" name="image_url" required value="{{ old('image_url') }}">
    @error('image_url')
    <p style="color: red;">{{ $message }}</p>
    @enderror
    ...
    <input type="checkbox" name="is_showing" value="1" id="is_showing" {{ old('is_showing') && old('is_showing') === "1" ? 'checked' : '' }}>
